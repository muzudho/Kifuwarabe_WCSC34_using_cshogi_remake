...
from v_a27_0_lib import Turn, Move, MoveHelper, BoardHelper, MoveListHelper, PolicyHelper, GameResultFile
from v_a27_0_eval_kk import EvaluationKkTable
from v_a27_0_eval_kp import EvaluationKpTable
...
""",
    ...
    def save_eval_all_tables(self):
        """（変更があれば）全ての評価値テーブル［0:先手, 1:後手］の保存"""
    ...
    def strengthen(
        ...
        if is_king_move:
            ...
            for kq_index, relation_exists in kq_index_to_relation_exists_dictionary.items():
                ...
                k_move_obj, q_move_obj = EvaluationKkTable.destructure_kq_index(
                        kp_index=kq_index)
                ...
                if k_move_obj.as_usi == move_u:
                    if relation_exists == 0:
                        ...
                        is_changed_temp = self._evaluation_kq_table_obj_array[Turn.to_index(self._board.turn)].set_relation_esixts_by_kl_moves(
                                k_move_obj=k_move_obj,
                                p_move_obj=q_move_obj,
                                is_rotate=self._board.turn==cshogi.WHITE,
                                bit=1)
