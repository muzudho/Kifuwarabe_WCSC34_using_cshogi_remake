# python v_a27_0_eval_p.py
from v_a27_0_lib import Move, BoardHelper
from v_a27_0_debug import DebugHelper
    ...
    def get_index_by_p_move(
        ...
        # 成らずだ
            ...
            try:
                ...
                # FIXME [unexpected error] err=KeyError(74), type(err)=<class 'KeyError'>
                # p_move_obj.as_usi:8a9c  src_sq:63  src_str:8a  promoted:False  dst_sq:74  ex:74

                # FIXME
                # [成らず] p_move_obj.as_usi:8a7c  src_sq:63  dst_sq:56  ex:56
                # [成らず] len(src_sq_to_dst_sq_to_index_for_npsi_dictionary):81
                # [成らず] len(src_sq_to_dst_sq_to_index_for_npsi_dictionary[p_move_obj.src_sq_or_none]):24

                # FIXME [2024-06-02 16:45:51.007274] [playout] start...
                # [成らず] p_move_obj.as_usi:8a7c  src_masu:81  dst_masu:73  ex:56
                # [成らず] len(src_sq_to_dst_sq_to_index_for_npsi_dictionary):81
                # [成らず] len(src_sq_to_dst_sq_to_index_for_npsi_dictionary[p_move_obj.src_sq_or_none]):24
                # [unexpected error] err=KeyError(56), type(err)=<class 'KeyError'>

                print(f"[成らず1] p_move_obj.as_usi:{p_move_obj.as_usi}  src_masu:{BoardHelper.sq_to_jsa(p_move_obj.src_sq_or_none)}  dst_masu:{BoardHelper.sq_to_jsa(p_move_obj.dst_sq)}  src_sq:{p_move_obj.src_sq_or_none}  dst_sq:{p_move_obj.dst_sq}  ex:{ex}")
                print(f"[成らず2] len(src_sq_to_dst_sq_to_index_for_npsi_dictionary):{len(src_sq_to_dst_sq_to_index_for_npsi_dictionary)}")
                if p_move_obj.src_sq_or_none < len(src_sq_to_dst_sq_to_index_for_npsi_dictionary):
                    print(f"[成らず3] len(src_sq_to_dst_sq_to_index_for_npsi_dictionary[p_move_obj.src_sq_or_none]):{len(src_sq_to_dst_sq_to_index_for_npsi_dictionary[p_move_obj.src_sq_or_none])}")

                    for dst_sq in src_sq_to_dst_sq_to_index_for_npsi_dictionary[p_move_obj.src_sq_or_none].keys():
                        print(f"[成らず4] dst_sq:{dst_sq}  dst_masu:{BoardHelper.sq_to_jsa(dst_sq)}")

                    if p_move_obj.dst_sq < len(src_sq_to_dst_sq_to_index_for_npsi_dictionary[p_move_obj.src_sq_or_none]):
                        print(f"[成らず5] len(src_sq_to_dst_sq_to_index_for_npsi_dictionary[p_move_obj.src_sq_or_none][p_move_obj.dst_sq]):{len(src_sq_to_dst_sq_to_index_for_npsi_dictionary[p_move_obj.src_sq_or_none][p_move_obj.dst_sq])}")
                raise
